...
\only<1->{
 \begin{textblock}{160}(0,22)
  \centering
  \only<1-3>{Creencia$(r,s=2|\text{M})$}\only<4->{Creencia$(r|s=2,\text{M})$} \\ \vspace{0.3cm}
 \begin{tabular}{c|c|c|c||c} \setlength\tabcolsep{0.4cm}
        & \, $r_1$ \, &  \, $r_2$ \, & \, $r_3$ \, &  \phantom{\bm{$1/3$}} \\ \hline
  &  &  &  & \\ \hline
  $s_2$ & \only<1-3>{$1/6$}\only<4>{$\frac{1}{6}/\frac{1}{3}$}\only<5>{$1/2$} & $0$ & \only<1-3>{$1/6$}\only<4>{$\frac{1}{6}/\frac{1}{3}$}\only<5>{$1/2$} & \only<2-3>{{$1/3$}}\only<4>{$\frac{1}{3}/\frac{1}{3}$}\only<5>{$1$} \\ \hline
  &  &  & &  \\
\end{tabular}
\end{textblock}
}
...
\begin{textblock}{160}(0,4)
 ...
 \begin{equation*}
  P(s_2) = \sum_i P(s_2|r_i) P(r_i) \onslide<3->{= \frac{1}{2} \frac{1}{3} + 0 \frac{1}{3} + \frac{1}{1} \frac{1}{3} } \onslide<4>{= 1/2}
 \end{equation*}
 \end{textblock}
}
\end{frame}
