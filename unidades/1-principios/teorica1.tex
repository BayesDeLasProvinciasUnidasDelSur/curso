...
\only<9->{
 ...
 No mentira inicial \small

Maximizando incertidumbre \\
...
\begin{frame}[plain]
 \begin{textblock}{160}(0,4)
 \centering \Large \only<-2>{Acuerdo inicial: el ``prior''}\only<3->{Nuevo acuerdo: el ``posterior''}
 \end{textblock}
...
\begin{equation*}
\ \phantom{\underbrace{\text{Creencia}(r|s_2,\text{M})}_{\text{Nueva creencia}} =} \hfrac{\overbrace{\text{Creencia}(r, s_2|\text{M})}^{\text{Creencia que sobrevive}}}{\phantom{\underbrace{\text{Creencia}(s_2|\text{M})}_{\text{Creencia total que sobrevive}}}}
\end{equation*}
\end{textblock}
}
...
\begin{textblock}{160}(0,58)
\begin{equation*}
\ \phantom{\underbrace{\text{Creencia}(r|s_2,\text{M})}_{\text{Nueva creencia}} =}\hfrac{\overbrace{\text{Creencia}(r, s_2|\text{M})}^{\text{Creencia que sobrevive}}}{\underbrace{\text{Creencia}(s_2|\text{M})}_{\text{Creencia total que sobrevive}}}
\end{equation*}
\end{textblock}
}
...
\begin{textblock}{160}(0,58)
\begin{equation*}
\underbrace{\text{Creencia}(r|s_2,\text{M})}_{\text{Nueva creencia}} = \frac{\overbrace{\text{Creencia}(r, s_2|\text{M})}^{\text{Creencia que sobrevive}}}{\underbrace{\text{Creencia}(s_2|\text{M})}_{\text{Creencia total que sobrevive}}}
\end{equation*}
\end{textblock}
}
...
\only<2->{
\begin{textblock}{80}(50,76)
$P(x,y,z) = P(x|y,z) \, \only<2>{P(y,z)}\only<3>{P(y|z) \, P(z)}$
\end{textblock}
}
...
\only<2>{
\begin{textblock}{160}(0,70)
\centering \Large ¿Y la verdad empírica respecto de los modelos?  \\[0.2cm] \large
...
\begin{textblock}{160}(0,4)
 \centering \Large
 $P(\text{Dato}|\text{Hipótesis})$ \\ \large Verosimilutd
 \end{textblock}
 \vspace{-1cm}


\begin{textblock}{80}(70,16) \centering
\scalebox{1.2}{
\tikz{
\only<3->{\phantom}{\node[latent, draw=white, yshift=0.8cm] (b0) {$1$};}
\only<8->{\phantom}{\node[latent,below=of b0,yshift=0.8cm, xshift=-2cm] (r1) {$r_1$};}
...
\only<8->{\phantom}{\edge[-] {br1} {r1d2,r1d3};}
\only<3-7,10-11>{\phantom}{\edge[-] {br2} {r2d3};}
\only<3-9>{\phantom}{\edge[-] {br3} {r3d2};}
...
\only<1>{
\begin{textblock}{120}(0,16) \centering
   \begin{equation*}
   P(r_i|s_2)= \frac{\overbrace{P(s_2|r_i)}^{\text{Verosimilutd}}P(r_i) }{P(s_2)} \onslide<2->{\propto P(r_i) P(s_2|r_i)} \onslide<3->{= P(r_i, s_2)}
  \end{equation*}
 \end{textblock}
}
...
\begin{textblock}{160}(0,4)
 \centering \Large
 $P(\text{Hipótesis}|\text{Dato})$ \\ \large Posterior
 \end{textblock}

 \begin{textblock}{160}(0,15)
  \begin{align*}
   \overbrace{P(r_i|s_2)}^{\text{Posterior}} = \frac{P(r_i) P(s_2|r_i)}{P(s_2)} \onslide<2->{\propto P(r_i) P(s_2|r_i)} \onslide<3->{= P(r_i, s_2)}
  \end{align*}
 \end{textblock}
...
\Large Lo inverosimil (\textbf{sorpresa}) como filtro de las creencias previas
\end{textblock}
}
...
\begin{textblock}{160}(0,4)
 ...
 \begin{equation*}
  P(s_2) = \sum_i P(s_2|r_i) P(r_i) \onslide<4->{= \frac{1}{2} \frac{1}{3} + 0 \frac{1}{3} + 1 \frac{1}{3} } \onslide<5>{= 1/2}
 \end{equation*}
 \end{textblock}
}
...
\Large Predicción con la contribución de todas las hipótesis.
\end{textblock}
}
