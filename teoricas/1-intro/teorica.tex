...
\tikz{         
    ...
    \node[const, right=of r] (dr) {\normalsize $ r_{ij} = (s_{ij}>0)$}; 

    \node[latent, above=of r, yshift=-0.45cm] (d) {$s_{ij}$} ; %
    \node[const, right=of d] (dd) {\normalsize $ s_{ij}=p_i-p_j$}; 
...
\begin{textblock}{160}(0,44)
 \begin{equation*}
  P(r_i|s_2) = \frac{P(r_i, s_2)}{P(s_2)} \onslide<2->{= \frac{\overbrace{P(s_2|r_i)}^{?}P(r_i)}{P(s_2)}}  \ \ \ \ \ \ \ \ \ \  \onslide<3->{P(s_2 | r_i) = \frac{P(r_i, s_2)}{P(r_i)}}
 \end{equation*}
\end{textblock}

\only<4-10>{
 \begin{textblock}{150}(0,70)
  \centering
  $P(s_2|r_i)$: \ \ \ 
 \begin{tabular}{c|c|c|c||c} \setlength\tabcolsep{0.4cm} 
          & \, $r_1$ \, &  \, $r_2$ \, & \, $r_3$ \, & \phantom{$1/2$} \\ \hline 
   $s_2$ & \onslide<6->{$1/2$} & \onslide<8->{$0$} & \onslide<10->{$1$} &  \\ \hline
\end{tabular} 
\end{textblock}
}
...
\begin{textblock}{160}(0,16)
  \centering
  \only<5>{$P(r_1, s)$}\only<6>{$P(s|r_1)$}\only<7>{$P(r_2, s)$}\only<8>{$P(s|r_2)$}\only<9>{$P(r_3, s)$}\only<10>{$P(s|r_3)$} \\ \vspace{0.1cm}
 \begin{tabular}{c|c|c|c||c} \setlength\tabcolsep{0.4cm} 
          & \, $r_1$ \, &  \, $r_2$ \, & \, $r_3$ \, &  \phantom{$1/2$} \\ \hline 
   $s_2$ & \only<5>{$1/6$}\only<6>{\bm{$1/2$}} & \only<7>{$0$}\only<8>{\bm{$0$}} & \only<9>{$1/3$}\only<10>{\bm{$1$}} &  \\ \hline
   $s_3$ & \only<5>{$1/6$}\only<6>{$1/2$} & \only<7>{$1/3$}\only<8>{$1$} & \only<9-10>{$0$}  &  \\ \hline
         & \only<5>{$1/3$}\only<6>{$1$} & \only<7>{$1/3$}\only<8>{$1$} & \only<9>{$1/3$}\only<10>{$1$}  &  \\ 
\end{tabular} 
\end{textblock}
}
...
\begin{textblock}{160}(0,4)
 ...
 \es{La sorpresa como filtro de las creencias previas}
 \end{textblock}
