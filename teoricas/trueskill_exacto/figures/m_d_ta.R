...
axis(lwd=1,side=1, at=ta,labels=expression( t[a] ),las=0,cex.axis=1.5,line=-0.3)

mtext(text ="Density" ,side =2 ,line=2,cex=2)
mtext(text =expression( t[b] ) ,side =1 ,line=3,cex=2)
abline(v=tb,lty=3)
...
mid <- length(D)%/%2
h <- dnormal[mid]/4
text(D[mid],h, expression(t[b]<t[a]), col = 1, cex=2.5)
