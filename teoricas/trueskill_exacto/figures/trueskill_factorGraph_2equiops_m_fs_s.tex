...
\node[det, fill=black!20] (r) {$r_1$};
	...
	\edge[-] {fd} {tb};
        ...
        \path (fs1) edge[bend left, arrow, color=red] node [right] {} (s1);
        \path (fs2) edge[bend left, arrow,color=red] node [right] {} (s2);	      
        \path (fs3) edge[bend right, arrow,color=red ] node [left] {} (s3);
        \path (fs4) edge[bend right, arrow, color=red] node [left] {} (s4);
\end{tikzpicture}
