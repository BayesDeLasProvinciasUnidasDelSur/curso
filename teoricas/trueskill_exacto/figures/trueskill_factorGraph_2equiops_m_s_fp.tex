...
\node[det, fill=black!20] (r) {$r_1$};
	...
	\edge[-] {fd} {tb};
        ...
        \path (s1) edge[bend left, arrow, color=red] node [right] {} (fp1);
        \path (s2) edge[bend left, arrow, color=red] node [right] {} (fp2);	      
        \path (s3) edge[bend right, arrow, color=red] node [left] {} (fp3);
        \path (s4) edge[bend right, arrow, color=red] node [left] {} (fp4);
\end{tikzpicture}
